    ...
    <div class="flexbox" style="height: 780px;">
        ...
        <div class="content" id="contentborder">
            <div id="adicionarpaciente">
                <header class="adicionarhead"><p>Alterando Paciente</p></header>
                <form action="alterar_paciente.php" method="post" id="adicionarpacienteform">
                <?php
                    $variavelId = $_GET['idpaciente'];
                    include "funcoes.php";
                    $conectar = conectarBancoDados();
                    
                    $sql = "SELECT * FROM pacientes WHERE id = '$variavelId'";
                    $stmt = $conectar->prepare($sql);
                    $stmt->execute();
                    $arrayMostrarPacientes = $stmt->fetchAll(PDO::FETCH_OBJ);

                    foreach($arrayMostrarPacientes as $pacientes) {
                        $nomeantigo = $pacientes->nome;
                        $cpfantigo = $pacientes->cpf;
                        $rgantigo = $pacientes->rg;
                        $nascimento = $pacientes->nacimento;
                        $sexo = $pacientes->sexo;
                        $enderecoantigo = $pacientes->endereco;
                        $emailantigo = $pacientes->email;
                        $celularantigo = $pacientes->celular;
                        $descricaoantiga = $pacientes->descricao;
                    }

                ?>
                    <input type="hidden" class="formulario" name="idpaciente" id="idpaciente" value="<?php echo($variavelId) ?>">
                    <div class="divinput">
                    ...
                    <div class="divinput">
                        <label for="endereco" class="labelform">Endereco</label><br>
                        <input type="text" class="formulario" name="endereco" id="endereco" value="<?php echo($enderecoantigo) ?>">
                        <p class="label-abaixo">Endereço Completo</p>
                    </div>
                    <div class="divinput">
